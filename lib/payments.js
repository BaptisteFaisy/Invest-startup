...
// Référence portée par le virement. Elle doit survivre au libellé d'un relevé
// bancaire (court, sans accent ni espace) et permettre de rapprocher un
// versement d'un compte sans ambiguïté. L'identifiant reste lisible pour le
// rapprochement manuel ; le suffixe aléatoire évite qu'un fondateur devine —
// et donc revendique — la référence d'un autre.
const BANK_TRANSFER_REFERENCE_RE = /^LQP-[1-9]\d*-[0-9A-F]{6}$/;

function bankTransferReference(userId, suffix) {
  const id = Number(userId);
  if (!Number.isInteger(id) || id <= 0) throw new Error('Identifiant utilisateur invalide');
  if (!/^[0-9A-Fa-f]{6}$/.test(String(suffix || ''))) throw new Error('Suffixe de référence invalide');
  return `LQP-${id}-${String(suffix).toUpperCase()}`;
}

// Extrait l'identifiant du compte depuis une référence. Retourne null si la
// référence n'a pas la forme attendue : un virement au libellé fantaisiste ne
// doit jamais activer un accès par erreur.
function userIdFromTransferReference(reference) {
  const value = String(reference || '').trim().toUpperCase();
  if (!BANK_TRANSFER_REFERENCE_RE.test(value)) return null;
